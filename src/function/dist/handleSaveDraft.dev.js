...
var handleSaveDraft = function handleSaveDraft(formData, setError, MIN_REGISTRATION_FEE) {
  if ((0, _validateCreateTournamentForm.validateForm)(formData, setError, MIN_REGISTRATION_FEE)) {
    ...
    try {
      ...
      addTableRow("Auction Date:", formatDate(formData.auctionDate), [37, 99, 235]); // 🔹 SECTION: Financial Info

      addSectionHeader("2. Financial Information");
      addTableRow("Registration Fee:", "$".concat(formData.registrationFee.toLocaleString()), [220, 38, 38]);
      addTableRow("Prize Money:", "$".concat(formData.prizeMoney.toLocaleString()), [234, 179, 8]);
      addTableRow("Total Budget:", "$".concat(formData.budget.toLocaleString()), [37, 99, 235]); // 🔹 SECTION: Budget Breakdown
